...
class View(GlobalView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        oRs = oConnExecute("SELECT battleid FROM battles_ships WHERE battleid=" + id + " AND owner_id=" + str(self.UserId) + " LIMIT 1")
        ...
        if not display_battle and self.AllianceId:
            if self.oAllianceRights["can_see_reports"]:
                ...
                oRs = oConnExecute("SELECT owner_id FROM battles_ships WHERE battleid=" + id + " AND (SELECT alliance_id FROM users WHERE id=owner_id)=" + str(self.AllianceId) + " LIMIT 1")
                display_battle = oRs != None
                if oRs:
        ...
        if display_battle:
            ...
            content = FormatBattle(id, creator, fromview, False)
