...
class View(GlobalView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        DisplayReport invasionid, self.UserId
    ...
    def DisplayReport(self, invasionid, readerid):
        ...
        query = "SELECT i.id, i.time, i.planet_id, i.planet_name, i.attacker_name, i.defender_name, " + \
                ...
                "i.def_workers_lost, galaxy, sector, planet, sp_get_user("+readerid+") " + \
        ...
        if oRs[4] != viewername and oRs[5] != viewername and not IsNull(self.AllianceId):
            ...
            if self.oAllianceRights["can_see_reports"]:
                ...
                query = "SELECT login" + \
                        " FROM users" + \
                        " WHERE (login="+dosql(oRs[4])+" OR login="+dosql(oRs[5])+") AND alliance_id="+self.AllianceId+" AND alliance_joined <= (SELECT time FROM invasions WHERE id="+invasionid+")"
                2
