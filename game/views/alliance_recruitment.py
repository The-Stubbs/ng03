...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.allianceRights["leader"] and not self.allianceRights["can_see_members_info"]: return HttpResponseRedirect("/game/alliance/")
        ...
        if cat == 1:
            ...
            if self.allianceRights["can_kick_player"]:
                if action == "kick":
                    ...
                    dbRow("SELECT user_alliance_kick_member("+str(self.userId)+","+sqlStr(self.username)+")")
        
        if cat == 2 and self.username != "":
            if self.allianceRights["can_invite_player"]:

                oRs = dbRow("SELECT user_alliance_invitation_create(" + str(self.userId) + "," + sqlStr(self.username) + ")")
                if oRs[0] == 0:
    ...
    def displayPage(self, cat):
        ...
        content = self.loadTemplate("alliance-members")
