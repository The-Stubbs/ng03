    ...
    def DisplayReport(self, invasionid, readerid):
        ...
        if oRs[4] != viewername and oRs[5] != viewername and self.allianceId:
            ...
            if self.allianceRights["can_see_reports"]:
                ...
                query = "SELECT login" + \
                        " FROM gm_profiles" + \
                        " WHERE (login="+sqlStr(oRs[4])+" OR login="+sqlStr(oRs[5])+") AND alliance_id="+str(self.allianceId)+" AND alliance_joined <= (SELECT time FROM gm_invasions WHERE id="+str(invasionid)+")"
                oRs2 = dbRow(query)
                if oRs2 == None:
                    return HttpResponseRedirect("/game/overview/")
                viewername = oRs2(0)
